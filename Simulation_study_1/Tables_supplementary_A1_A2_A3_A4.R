...
b1.TGMRF <- lapply(TGMRF.GSC, beta1.mean.nimble)
...
Table.MARB.beta1 <- data.frame(Null=MARB.beta1(b1.Null), Spat=MARB.beta1(b1.Spat),
                               RSR=MARB.beta1(b1.RSR), TGMRF1=MARB.beta1(b1.TGMRF),
...
Table.MRRMSE.beta1 <- data.frame(Null=MRRMSE.beta1(b1.Null), Spat=MRRMSE.beta1(b1.Spat),
                                 RSR=MRRMSE.beta1(b1.RSR), TGMRF1=MRRMSE.beta1(b1.TGMRF),
...
colnames(Table.MARB.beta1) <- c("Null", "Spatial", "RSR", "TGRMF.GSC", 
...
Table.Accuracy[, 1:12] <- round(Table.Accuracy[, 1:12], 4)
...
Length.beta1.nimble <- function(Model){
  data.frame(length=abs(Model$quantiles[grep('X1', rownames(Model$quantiles), value=TRUE), 5] - Model$quantiles[grep('X1', rownames(Model$quantiles), value=TRUE), 1]))
}
...
Length <- data.frame(Null=mean(unlist(lapply(Null, function(x) Length.beta1(x)))),
                     ...
                     TGMRF1=mean(unlist(lapply(TGMRF.GSC, function(x) Length.beta1.nimble(x)))),
...
TGMRF.DIC <- apply(do.call(rbind, WAIC.GSC), 2, mean)


Table.DIC <- rbind(Null.DIC, Spat.DIC, RSR.DIC, c(NA,NA,NA,TGMRF.DIC), SpatPlus5.DIC, SpatPlus10.DIC,
...
rownames(Table.DIC) <- c("Null", "Spatial", "RSR", "TGRMF1", 
                         "SpatPlus5", "SpatPlus10", "SpatPlus15", "SpatPlus20",
...
RR.TGMRF <- lapply(TGMRF.GSC, function(x) RR.nimble(x))
...
Table.MARB.RR <- data.frame(Null=MARB.RR(RR.Null), Spat=MARB.RR(RR.Spat),
                   RSR=MARB.RR(RR.RSR), TGMRF1=MARB.RR(RR.TGMRF),
...
Table.MRRMSE.RR <- data.frame(Null=MRRMSE.RR(RR.Null), Spat=MRRMSE.RR(RR.Spat),
                              RSR=MRRMSE.RR(RR.RSR), TGMRF1=MRRMSE.RR(RR.TGMRF),
...
colnames(Table.MARB.RR) <- c("Null", "Spatial", "RSR", "TGRMF1", 
                            "SpatPlus5", "SpatPlus10", "SpatPlus15", "SpatPlus20",
                            "SpatPlusP1", "SpatPlusTP1", "SpatPlusP2", "SpatPlusTP2")
